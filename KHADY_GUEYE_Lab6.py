...
""" The three function have buggs in them, I must find them and get the expected output. """
...
def find_second_largest(numbers):
    ...
    for num in numbers:
        if num > largest:
            second_largest = largest  # Store previous largest
...
def three_sum(numbers, n):
    ...
    for i in range(len(numbers)):
        for j in range(i + 1, len(numbers)):  # Start from i+1
            for k in range(j + 1, len(numbers)):  # Start from j+1
                if numbers[i] + numbers[j] + numbers[k] == n:
                    return True  # If three numbers sum to n, return True
    return False
